...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOfPlanes();
	}

    function updateNumberOfPlanes() {
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        Plane.position.x = win.shape.x + (win.shape.w * .5);
        Plane.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < Planes.length; i++) {
			...
			Plane.position.x = Plane.position.x + (posTarget.x - Plane.position.x) * falloff;
			Plane.position.y = Plane.position.y + (posTarget.y - Plane.position.y) * falloff;
			Plane.rotation.x = _time * .5;
			Plane.rotation.y = _time * .3;
		};
