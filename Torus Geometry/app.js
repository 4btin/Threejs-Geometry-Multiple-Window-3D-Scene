...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOftoruss();
	}

    function updateNumberOftoruss() {
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        torus.position.x = win.shape.x + (win.shape.w * .5);
        torus.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < toruss.length; i++) {
			...
			torus.position.x = torus.position.x + (posTarget.x - torus.position.x) * falloff;
			torus.position.y = torus.position.y + (posTarget.y - torus.position.y) * falloff;
			torus.rotation.x = _time * .5;
			torus.rotation.y = _time * .3;
		};
