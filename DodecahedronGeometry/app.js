...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOfDodecahedrons();
	}

    function updateNumberOfDodecahedrons() {
    let wins = windowManager.getWindows();
    Dodecahedrons.forEach((c) => {
        world.remove(c);
    })
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        Dodecahedron.position.x = win.shape.x + (win.shape.w * .5);
        Dodecahedron.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < Dodecahedrons.length; i++) {
			let Dodecahedron = Dodecahedrons[i];
			let win = wins[i];
			let _time = time;
			let posTarget = {
			...
			Dodecahedron.position.x = Dodecahedron.position.x + (posTarget.x - Dodecahedron.position.x) * falloff;
			Dodecahedron.position.y = Dodecahedron.position.y + (posTarget.y - Dodecahedron.position.y) * falloff;
			Dodecahedron.rotation.x = _time * .5;
			Dodecahedron.rotation.y = _time * .3;
		};
