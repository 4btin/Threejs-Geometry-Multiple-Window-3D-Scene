...
if (new URLSearchParams(window.location.search).get("clear")) {
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        capsule.position.x = win.shape.x + (win.shape.w * .5);
        capsule.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < capsules.length; i++) {
			...
			capsule.position.x = capsule.position.x + (posTarget.x - capsule.position.x) * falloff;
			capsule.position.y = capsule.position.y + (posTarget.y - capsule.position.y) * falloff;
			capsule.rotation.x = _time * .5;
			capsule.rotation.y = _time * .3;
		};
