...
if (new URLSearchParams(window.location.search).get("clear")) {
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        torusKnot.position.x = win.shape.x + (win.shape.w * .5);
        torusKnot.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < torusKnots.length; i++) {
			...
			torusKnot.position.x = torusKnot.position.x + (posTarget.x - torusKnot.position.x) * falloff;
			torusKnot.position.y = torusKnot.position.y + (posTarget.y - torusKnot.position.y) * falloff;
			torusKnot.rotation.x = _time * .5;
			torusKnot.rotation.y = _time * .3;
		};
