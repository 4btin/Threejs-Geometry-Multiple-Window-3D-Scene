...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOflathes();
	}

    function updateNumberOflathes() {
    ...
    for (let i = 0; i < wins.length; i++) {
		...
		lathe.scale.set(scaleFactor, scaleFactor, scaleFactor);
        lathe.position.x = win.shape.x + (win.shape.w * .5);
        lathe.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < lathes.length; i++) {
			...
			lathe.position.x = lathe.position.x + (posTarget.x - lathe.position.x) * falloff;
			lathe.position.y = lathe.position.y + (posTarget.y - lathe.position.y) * falloff;
			lathe.rotation.x = _time * .5;
			lathe.rotation.y = _time * .3;
		};
