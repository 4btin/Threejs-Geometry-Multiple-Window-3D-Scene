...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOfoctahedrons();
	}

    function updateNumberOfoctahedrons() {
    let wins = windowManager.getWindows();
    octahedrons.forEach((c) => {
        world.remove(c);
    })
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        octahedron.position.x = win.shape.x + (win.shape.w * .5);
        octahedron.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < octahedrons.length; i++) {
			...
			octahedron.position.x = octahedron.position.x + (posTarget.x - octahedron.position.x) * falloff;
			octahedron.position.y = octahedron.position.y + (posTarget.y - octahedron.position.y) * falloff;
			octahedron.rotation.x = _time * .5;
			octahedron.rotation.y = _time * .3;
		};
