...
if (new URLSearchParams(window.location.search).get("clear")) {
	...
	function windowsUpdated() {
		updateNumberOftetrahedrons();
	}

    function updateNumberOftetrahedrons() {
    let wins = windowManager.getWindows();
    tetrahedrons.forEach((c) => {
        world.remove(c);
    })
    ...
    for (let i = 0; i < wins.length; i++) {
        ...
        tetrahedron.position.x = win.shape.x + (win.shape.w * .5);
        tetrahedron.position.y = win.shape.y + (win.shape.h * .5);
	...
	function render() {
		...
		for (let i = 0; i < tetrahedrons.length; i++) {
			let tetrahedron = tetrahedrons[i];
			let win = wins[i];
			let _time = time;
			let posTarget = {
			...
			tetrahedron.position.x = tetrahedron.position.x + (posTarget.x - tetrahedron.position.x) * falloff;
			tetrahedron.position.y = tetrahedron.position.y + (posTarget.y - tetrahedron.position.y) * falloff;
			tetrahedron.rotation.x = _time * .5;
			tetrahedron.rotation.y = _time * .3;
		};
